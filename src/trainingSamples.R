    ...
    sub = "") 
{
    ...
    if (length(probVar) > 0) {
        ...
        warnString <- paste("The models", paste(probModel, 
            collapse = ", "), "are not in the lift chart because the variables", 
    ...
    if (type == "cumulative") {
        ...
        for (i in 1:nmodels) {
            ...
            if (modtype == "glm" | modtype == "nnet.formula") {
                ...
                var1 <- yvar[order(as.vector(predict(model, newdata = data)[, 
                  targLevel]), decreasing = TRUE)]
            }
            ...
            lines(seq(0.1, 1, 0.1), cumsum(var2)/sum(var2), col = colr[i], 
                lwd = 2)
        ...
        for (j in 1:nmodels) {
            lines(seq(0.1, 1, 0.1), as.vector(resp.matrix[, j]), 
                col = colr[j], lwd = 2)
